...
const AlgosCompleted = (props) => {
    return (
        <div>
                ...
                {props.completedproblems.length && props.completedproblems.map(algo => {
                    let time = calculateTimeToDisplay(algo.time)
                    return(
                    <p key={algo.id}>{algo.title} - Level: {algo.level},  Time To solve: {time},  Average Percentile: {algo.average_precentile}</p>
                )})}
            </div>
    )
}
