...
class Problems extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      if (this.props.currentTag === 'Any') {
        res = await fetch('/api/tags/any', { method: 'GET' });
      } else {
        res = await fetch(`/api/tags/${this.props.currentTag}`, {
          method: 'GET',
        });
      }
  ...
  handlesubmit(evt) {
    ...
    thisProblem.average_precentile = Math.floor(
      (thisProblem.runtime_percentile + thisProblem.memory_percentile) / 2
    );
    ...
    if (this.props.current > 60) {
      ...
      let alreadyCompleted = pastProblems.filter(
        (algo) => algo.id === nextProblem.id
      );
    ...
    return (
      <div>
        <h2>
          ...
          <a href={`https://leetcode.com/problems/${name}`} target="_blank">
            Leetcode Link
          </a>
        </h2>
        <p>
          Start Time: {startTime} - Level: {this.state.currentProblem.level} -
          Acceptance: {this.state.currentProblem.acceptance} - Frequency:{' '}
          {this.state.currentProblem.frequency}
        </p>
        <label htmlFor="runtime">
          Runtime:{' '}
          <input
            type="Number"
            value={this.state.currentProblem.runtime}
            onChange={this.handleChange}
            name="runtime"
          />
        </label>
        <label htmlFor="runtime_precentile">
          Runtime %:{' '}
          <input
            type="Number"
            value={this.state.currentProblem.runtime_percentile}
            onChange={this.handleChange}
        ...
        <label htmlFor="memory">
          ...
          <input
            type="Number"
            value={this.state.currentProblem.memory}
            onChange={this.handleChange}
            name="memory"
          />
        </label>
        <label htmlFor="memory_precentile">
          Memory %:{' '}
          <input
            type="Number"
            value={this.state.currentProblem.memory_percentile}
            onChange={this.handleChange}
