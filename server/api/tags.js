...
router.get('/any', async (req, res, next) => {
  try {
    const data = await client.query(
      'SELECT * FROM problems ORDER BY level ASC LIMIT 15'
    );
...
router.get('/:tag', async (req, res, next) => {
  try {
    const data = await client.query(
      'SELECT * FROM problems, algoTagged WHERE algoTagged.tag = $1 AND problems.id = algoTagged.algo_id ORDER BY level ASC LIMIT 15',
      [req.params.tag]
    );
    const algos = data.rows;
    // if(algos.length < 10){
